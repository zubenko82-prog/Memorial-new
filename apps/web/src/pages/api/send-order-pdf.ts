...
// Принимает PDF и отправляет в Telegram-чат(ы) менеджеров.
// Возвращает подробный ответ Telegram (message_id, chat.id) + версию.
// Env (в веб‑проекте!):
// - TGBOT_TOKEN
// - MANAGER_CHAT_ID  (или MANAGER_CHAT_IDS через запятую)
// - ALLOW_ORIGIN (опционально для CORS)
...
const VERSION = "send-order-pdf@2026-01-12-2";
...
async function sendToTelegramDocument(params: {
  ...
  // @ts-expect-error Node 18+ глобальный File доступен
  ...
  let json: TgResult | null = null;
  try { json = text ? (JSON.parse(text) as TgResult) : null; } catch { json = null; }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const chats: string[] = (chatMulti ? chatMulti.split(",") : []).map((s) => s.trim()).filter(Boolean);
  ...
  try {
    ...
    const metaRaw = typeof fields.payload === "string"
      ? fields.payload
      : Array.isArray(fields.payload) ? fields.payload[0] : "{}";
