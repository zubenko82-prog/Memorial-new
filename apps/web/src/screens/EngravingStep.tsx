...
//
// Требования:
// - Тяжёлые фото сжимаем до безопасного размера перед сохранением (локально, через canvas).
// - Драфт НЕ обновляем «на лету». Пишем только при переходах/размонтировании/закрытии вкладки.
//
// Что сделано:
// - Добавлен локальный компрессор изображений (без внешних зависимостей).
// - При выборе фото сразу показываем transient-превью (blob:), параллельно сжимаем и сохраняем в state.
// - Драфт сохраняем в saveOrderDraft ТОЛЬКО:
//     • при нажатии «Назад»/«Продолжить»,
//     • при размонтировании компонента,
//     • при beforeunload/pagehide/visibilitychange (уход со страницы),
//     • при hashchange/popstate (переход между шагами).
// - Подтягиваем внешние изменения драфта и сбрасываем локальное состояние при очистке.
// - Исправлена опечатка initial!.епитaphText → initial!.epitaphText.
...
type Person = {
  ...
  birthDate?: string;
  deathDate?: string;
  photoUrl?: string | null;     // «сылая» ссылка или внешний URL (для превью)
  photoDataUrl?: string | null; // сжатый dataURL, готовый к отправке
};
...
/* ===== Локальный компрессор изображений ===== */
// целевое ограничение ≈2.7 MiB — безопасно для Telegram
const PHOTO_TARGET_MAX_BYTES = Math.floor(2.7 * 1024 * 1024);
const MAX_DIM = 2200; // пикселей по длинной стороне
...
async function compressImageFileToMaxBytes(file: File, maxBytes = PHOTO_TARGET_MAX_BYTES): Promise<Blob> {
  const image = await loadImageFromFileOrBlob(file);
  // начальные размеры
  const iw = image.naturalWidth || image.width;
  const ih = image.naturalHeight || image.height;
  const r = iw / ih;
  // целевые размеры по длинной стороне
  let tw = iw;
  let th = ih;
  ...
  // подбор качества
  let q = QUALITY_START;
  let out: Blob = await new Promise((resolve) => canvas.toBlob((b) => resolve(b || new Blob()), "image/jpeg", q));
  if (out.size <= maxBytes) return out;

  while (q > QUALITY_MIN) {
    q = Math.max(QUALITY_MIN, q - QUALITY_STEP);
    out = await new Promise((resolve) => canvas.toBlob((b) => resolve(b || new Blob()), "image/jpeg", q));
    if (out.size <= maxBytes) return out;
  }

  // если всё ещё велик — уменьшаем геометрию и пробуем снова
  let scale = 0.9;
  for (let i = 0; i < 4 && out.size > maxBytes; i++) {
    ...
    while (q > QUALITY_MIN && out.size > maxBytes) {
      q = Math.max(QUALITY_MIN, q - QUALITY_STEP);
      out = await new Promise((resolve) => c2.toBlob((b) => resolve(b || new Blob()), "image/jpeg", q));
    }
    scale *= 0.9;
  }
  return out;
...
export default function EngravingStep({
  ...
  const removePerson = (idx: number) => setPersons((prev) => {
  ...
  const moveDown = (idx: number) => setPersons((prev) => (idx === prev.length - 1 ? prev : prev.map((x, i) => (i === idx ? prev[idx + 1] : i === idx + 1 ? prev[idx] : x))));

  /* ===== Фото: сжатие «тяжёлых» изображений перед сохранением ===== */
    ...
    if (maybeFile instanceof File) {
      ...
      (async () => {
        try {
          ...
          commitLocal({ photoDataUrl: out, photoUrl: out });
        } catch <button type="button" onClick={handleBack} style={glassButtonStyle("sm")}>Назад</button>
          <button
            type="button"
