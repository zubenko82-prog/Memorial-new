...
async function uploadFileInChunksToBlob(params: {
  file: File;
  blobPathname: string;
  contentType: string;
  chunkBytes?: number;
  ...
  const chunkBytes = params.chunkBytes ?? 650_000;
  ...
  while (offset < buf.length) {
    const slice = buf.subarray(offset, Math.min(buf.length, offset + chunkBytes));
    const chunkBase64 = uint8ToBase64(slice);

    const r = await fetch("/api/upload-chunk", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ uploadId, chunkBase64 })
    });
    ...
    if (!r.ok || !j?.ok) throw new Error(j?.error || raw || "chunk upload failed");

    offset += slice.length;
    params.onProgress?.(Math.min(1, offset / total));
    await sleep(5);
  }

  const r2 = await fetch("/api/upload-chunk", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      uploadId,
      final: "1",
      pathname: params.blobPathname,
  ...
  if (!r2.ok || !j2?.ok) throw new Error(j2?.error || raw2 || "finalize failed");
...
export default function ReviewAndSendStep({
  ...
  function buildOrderText(): string {
    ...
    if (frontEpitaphs2.length) {
      lines.push("- Эпитафии:");
      frontEpitaphs2.forEach((tx: string, i: number) => lines.push(`  ${i + 1}) ${tx}`));
    } else {
      lines.push("- Эпитафии: —");
    }
    ...
    if (rearEnabled) {
      ...
      if (rearEpitaphs.length) {
        lines.push("- Эпитафии:");
        rearEpitaphs.forEach((tx: string, i: number) => lines.push(`  ${i + 1}) ${tx}`));
      } else {
        lines.push("- Эпитафии: —");
      }
    ...
    if (plateEnabled && p1PreviewUrl) {
      ...
      if (plateEpitaphs.length) {
        lines.push("- Эпитафии:");
        plateEpitaphs.forEach((tx: string, i: number) => lines.push(`  ${i + 1}) ${tx}`));
      } else {
        lines.push("- Эпитафии: —");
      }
  ...
  const sendOrderDirect = async () => {
    ...
    try {
      ...
      // ===== EMAIL via Blob (chunk upload) + прогресс =====
      try {
        setEmailProgress(3);
        const orderText = buildOrderText();

        const plateNodesLocal = showPlate
          ? plateToShow.map((p) => document.getElementById(`pdf-plate-sketch-${p.index}`))
          : [];
        ...
        const personNamedPhotos: NamedFile[] = collectPersonPhotosWithCaptions(loadOrderDraft()).map((x) => ({
          file: x.file,
          name: safeBlobName(x.name)
        }));

        const sketchFiles = await collectSketchFilesForEmail(orderNoCur);
        const allPhotos: NamedFile[] = [...personNamedPhotos, ...sketchFiles].filter((x) => !!x?.file);

        const baseFolder = `orders/${encodeURIComponent(orderNoCur)}`;

        const pdfUploaded = await uploadFileInChunksToBlob({
          file: pdfFile,
          blobPathname: `${baseFolder}/pdf/${encodeURIComponent(pdfFile.name)}`,
          contentType: "application/pdf",
          onProgress: (p) => setEmailProgress(8 + Math.round(p * 22)) // 8..30
        });
        ...
        for (let i = 0; i < allPhotos.length; i++) {
          ...
          const safeName = safeBlobName(nf.name || `photo-${i + 1}.jpg`) || `photo-${i + 1}.jpg`;

          const uploaded = await uploadFileInChunksToBlob({
            file: new File([nf.file], safeName, { type: nf.file.type || "image/jpeg" }),
            blobPathname: `${baseFolder}/photos/${encodeURIComponent(safeName)}`,
            contentType: nf.file.type || "image/jpeg",
            onProgress: (p) => {
              const overall = (i + p) / Math.max(1, allPhotos.length);
              setEmailProgress(30 + Math.round(overall * 55)); // 30..85
            }
          });

          photoUrls.push(uploaded.url);
          photoPathnames.push(uploaded.pathname);
          photoFilenames.push(safeName);
        ...
        if (!mailRes.ok) {
          ...
          warnings.push(`Email не отправлен: ${mailRes.error || "ошибка"}`);
        } else {
  ...
  async function handleSavePdf() {
    try {
      ...
      const plateUrlFallbacks2 = showPlate ? plateToShow.map((p) => p.url) : [];
      ...
      const blob = await generateOrderPdfShots({
        ...
        plateNodes,
        plateUrlFallbacks: plateUrlFallbacks2,
  ...
  const overlayText =
    uploading ? `Отправляем… TG:${Math.max(0, Math.min(100, uploadProgress || 0))}% / Email:${Math.max(0, Math.min(100, emailProgress || 0))}%` :
              ...
              <div style={{ display: "grid", gap: 6 }}>
                ...
                <div>
                  ...
                  <strong
                    style={{
                      color:
                        photosDelivered === photosTotal ? "#7dffa0" : photosDelivered > 0 ? "#ffd666" : photosTotal === 0 ? "#ccc" : "#ffb4b4"
                    }}
                  >
                ...
                <div>
                  ...
                  <strong style={{ color: emailDelivered == null ? "#ffd666" : emailDelivered ? "#7dffa0" : "#ffb4b4" }}>
                    {emailDelivered == null ? `в процессе… ${emailProgress}%` : emailDelivered ? "да" : "нет"}
                  </strong>
                </div>
              </div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: 0, fontSize: 15 }}>
              Перед началом новой заявки <span style={{ color: "#c00" }}>настоятельно рекомендуем</span> скачать и сохранить PDF
              заказа&nbsp;— иначе ваши данные будут безвозвратно утеряны.
            </div>
            ...
            <button
              style={glassButtonStyle("sm")}
              onClick={() => {
                const mode = showWipeWarn;
                setShowWipeWarn(null);

                if (mode === "wipeAll") {
                  void hardResetAll({ preserveThemeKey: true });
                  return;
                }
                if (mode === "wipeKeepCustomer") {
                  onNewOrderWipeKeepCustomer?.();
                  return;
                }
                if (mode === "wipeKeepAll") {
      ...
      {confirmOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 5 }}>Отправить заказ менеджерам?</div>
