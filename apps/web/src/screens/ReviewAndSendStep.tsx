...
// - Порядок блоков: ссылка -> TopBar -> Выбрано для плиты (если есть) -> Аккордеоны «Дополнительно / Надгробная плита»
//   -> Эскизы лицевая/тыльная -> Комментарий/подсказка -> Кнопки «Назад / Рассчитать стоимость».
// - Кнопка «Рассчитать стоимость» открывает bottom sheet с кнопками «Отправить» и «Сохранить PDF».
// - PDF (1512×2138): слева состав заказа (с миниатюрами!), справа — эскизы (лицевой над тыльным, друг над другом);
//   далее — прикреплённые фото (по одному на страницу).
// - PDF содержит имя, телефон и № заказа. Фото берём из p.photoPreview / p.photoDataUrl / p.photoUrl / p.photo и конвертируем в dataURL.
// - Исправлено: корректная вёрстка аккордеонов — без ошибок закрывающих тегов.
...
export default function ReviewAndSendStep({ onBack }: Props) {
  ...
  return (
    <div style={safeRoot()}>
      {/* ОДНА ссылка для разворота TopBar */}
