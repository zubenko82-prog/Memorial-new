...
// - Не отправляем PDF в Telegram (часто упирается в лимиты). Вместо него:
//   1) Отправляем полное текстовое описание заказа (бьём на части ≤ 4096, используем ~3500).
//   2) Отправляем эскизы (лицевая/тыльная) как изображения. Если DOM→PNG не удаётся (CORS),
//      для тыльной шлём напрямую URL (сервер отправит в Telegram photo=URL).
//   3) Обязательно отправляем портретные фотографии по отдельности с подписями (ФИО и даты).
// - Кнопка «Скачать PDF» генерирует локальный PDF со всеми фото (для клиента, не в Telegram).
// - Блок «Заявка отправлена» объединён со «Статус доставки» и расположен внизу.
// - Блок «Выбрано для плиты» восстановлен, добавлены «Размер» и «Ширина», расположен над «Комментарий к заказу».
// - Добавлены маркеры начала/конца заявки в Telegram (декоративные сообщения с пиктограммами).
...
export default function ReviewAndSendStep({ onBack }: { onBack?: () => void }) {
  ...
  // Маркеры начала/конца в Telegram
  ...
  function buildOrderText(): string {
    ...
    if (gfxRear.length)  { lines.push("Графика (тыльная):"); gfxRear.forEach((n) => {
  ...
  async function sendSketchFromNode(nodeId: string, caption: string, fallbackUrl?: string | null): Promise<{ ok: boolean; error?: string }> {
    ...
    // 2) Fallback: отправим URL (Telegram сам скачает)
    try {
      if (fallbackUrl) {
  ...
  // Основная отправка (маркеры + текст + эскизы + фото)
    ...
    <div style={safeRoot()}>
      ...
      <section style={{ ...glassPanelStyle(), padding: 10, marginTop: 10 }}>
        <PlateBlock
          ...
          plateIds={plateIds}
          hasPedestal={hasPedestal} setHasPedestal={(v) => { setHasPedestal(v); const prev = loadOrderDraft(); saveOrderDraft({ ...prev, extras: { ...(prev as any).extras, tumba: v }, updatedAt: Date.now() }); }}
          hasFlowerbed={hasFlowerbed} setHasFlowerbed={(v) => { setHasFlowerbed(v); const prev = loadOrderDraft(); saveOrderDraft({ ...prev, extras: { ...(prev as any).extras, flowerbed: v }, updatedAt: Date.now() }); }}
          hasVase={hasVase} setHasVase={(v) => { setHasVase(v); const prev = loadOrderDraft(); saveOrderDraft({ ...prev, extras: { ...(prev as any).extras, vase: v }, updatedAt: Date.now() }); }}
