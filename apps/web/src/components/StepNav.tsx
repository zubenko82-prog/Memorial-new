...
export type StepNavProps = {
  ...
  // compatibility props (ignored, but may be passed)
  sticky?: boolean;
...
function Icon({ id }: { id: string }) {
  ...
  switch (id) {
    case "item":
      return (
        <svg {...p} aria-hidden>
          <path d="M4 6a2 2 0 0 1 2-2h7l5 5v9a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6zm10-1v4h4" />
        </svg>
      );
    case "params":
      return (
    ...
    case "persons":
      return (
        <svg {...p} aria-hidden>
          <path d="M16 11a4 4 0 1 0-8 0 4 4 0 0 0 8 0ZM4 20a8 8 0 1 1 16 0" stroke="currentColor" strokeWidth="2" fill="none" />
        </svg>
      );
    case "graphics":
      return (
        <svg {...p} aria-hidden>
          <path d="M4 5h16v14H4zM6 15l4-5 3 4 2-3 3 4" stroke="currentColor" strokeWidth="2" fill="none" />
        </svg>
      );
    case "epitaph":
      return (
        <svg {...p} aria-hidden>
          <path d="M6 7h12M6 12h10M6 17h8" stroke="currentColor" strokeWidth="2" fill="none" />
        </svg>
      );
    case "editor":
      return (
    ...
    case "rear":
      return (
        <svg {...p} aria-hidden>
          <path d="M5 6h9v12H5z" stroke="currentColor" strokeWidth="2" fill="none" />
          <path d="M10 6h9v12h-9z" />
        </svg>
      );
    case "review-and-send":
      return (
        <svg {...p} aria-hidden>
          <path d="M6 6h12v12H6z" stroke="currentColor" strokeWidth="2" fill="none" />
...
// В вашем App "review" мапится на StepId="finish"
...
export default function StepNav({
  ...
  const [enabled, setEnabled] = useState<boolean>(() => {
    ...
    try {
      return persistKey ? window.localStorage.getItem(persistKey) === "1" : false;
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    if (typeof enabledProp === "boolean") return; // controlled outside
  ...
  useLayoutEffect(() => {
    ...
    if (!enabled) {
      try {
        document.documentElement.style.setProperty(heightCssVar, "0px");
      } catch {}
      return;
    }
  ...
  // For fixed: render via portal into body so it won't be affected by scroll container transforms
